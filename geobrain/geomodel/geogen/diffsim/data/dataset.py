...
def make_dataset(dir):
    ...
    if os.path.isfile(dir):
        ...
        for root, dirnames, fnames in os.walk(dir):
...
class InpaintDatasetCase1(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        cond_image = torch.cat((1. - mask, sandcond, bankcond, mudcond), dim=0).float()
...
class InpaintDatasetCase2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mudcond = (init_cond_image == -1) + 0.0
        cond_image = torch.cat((1. - mask, sandcond, sbankcond, smudcond, mudcond), dim=0).float()
