...
High-level interface for acoustic wave simulation combining
...
from ..core import AbstractModel, AbstractPropagator, WaveResult
from ..models import AcousticModel
from ..survey import Survey
from ..boundary import create_boundary
from ..kernels import acoustic_forward_kernel, acoustic_forward_kernel_3d
...
class AcousticPropagator(AbstractPropagator):
    ...
    Solves the acoustic wave equation using finite difference
    method with 4th-order staggered grid scheme.
    
    Args:
        model: Acoustic velocity model.
    ...
    Example:
        Basic usage:
        >>> model = AcousticModel(grid, boundary, vp=vp, rho=rho)
        ...
        >>> result = propagator.forward(shot_index=0)
    """
    ...
    def __init__(
        ...
        if not isinstance(model, AcousticModel):
            raise TypeError(
                f"Expected AcousticModel, got {type(model).__name__}"
            )
    ...
    def forward(
        self,
        model: Optional[AcousticModel] = None,
        ...
        Returns:
            ...
            Supports dict-like access: ``result['p']`` still works.

        Example:
            All shots:
            >>> result = propagator.forward(checkpoint_segments=4)
            >>> seismograms = result.p  # attribute access

            Single shot:
            >>> result = propagator.forward(shot_index=0)

            With updated model:
            >>> result = propagator.forward(model=updated_model)
        """
        model = model if model is not None else self.model
        model.forward()  # Prepare model
        ...
        # Run forward kernel
        if self.model.ndim == 3:
            result = acoustic_forward_kernel_3d(
                nx=self.nx,
                ny=self.ny,
                nz=self.nz,
                dx=self.dx,
